...
def run_scheduled_analysis(
    ...
    result = run_full_analysis(config, args, stock_codes, raise_errors=True)
    # MA 趋势 Agent 已改用独立的 RuntimeScheduler 后台任务（MA_AGENT_SCHEDULE_TIME）
    # 不再从主分析调度中调用，避免启动 run_immediately 时重复触发。
    return result
...
def main() -> int:
    ...
    if start_serve:
        ...
        runtime_schedule_requested = automation_enabled() and not args.serve_only and (
    ...
    try:
        ...
        # 模式2: 定时任务模式
        if automation_enabled() and (args.schedule or config.schedule_enabled):
            if start_serve:
