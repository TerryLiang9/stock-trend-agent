...
class TestIsCodeLike:
    ...
    def test_us_ticker(self):
        assert is_code_like("AAPL") is False

    def test_us_ticker_with_exchange(self):
        assert is_code_like("TSLA.O") is False
...
class TestNormalizeCode:
    ...
    def test_prefix_hk(self):
        assert normalize_code("HK00700") is None
    ...
    # --- Non-A-share tickers ---
    def test_us_ticker(self):
        assert normalize_code("AAPL") is None
...
class TestResolveIndexStockCodeForAnalysis:
    def test_accepts_plain_a_share(self):
        assert resolve_index_stock_code_for_analysis("600519") == "600519"

    def test_rejects_indexed_4_digit_jp_base(self):
        assert is_code_like("7203") is False
        assert resolve_index_stock_code_for_analysis("7203") == ""

    def test_rejects_non_a_share(self):
        assert resolve_index_stock_code_for_analysis("005930.KS") == ""
        assert resolve_index_stock_code_for_analysis("AAPL") == ""
