...
export const useAgentChatStore = create<AgentChatState & AgentChatActions>((set, get) => {
  ...
  startStream: async (payload, meta) => {
    ...
    try {
      ...
      const acceptedValidationError = (event: Record<string, unknown>) => {
        const details = [
          `expected backend: litellm / codex_app_server, got: ${String(event.backend)}`,
          `expected request_id: ${requestId}, got: ${String(event.request_id)}`,
          `expected session_id: ${streamSessionId}, got: ${String(event.session_id)}`,
        ].join('; ');
        return protocolError(`Agent stream emitted an invalid accepted event. ${details}`);
      };
      const processLine = (line: string) => {
        // Normalise CRLF / trailing whitespace.
        const cleaned = line.replace(/\r$/, '').trimEnd();
        if (!cleaned.startsWith('data: ') || !ownsStream() || ac.signal.aborted) return;

        let event: ProgressStep;
        try {
          event = JSON.parse(cleaned.slice(6)) as ProgressStep;
        } catch {
          // Malformed JSON in a data line — ignore and wait for a valid event.
          return;
        }
        if (event.type === 'accepted') {
          ...
          if (
            ...
            throw acceptedValidationError(event as unknown as Record<string, unknown>);
          }
