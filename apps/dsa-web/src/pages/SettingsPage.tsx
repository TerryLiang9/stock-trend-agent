...
import TrendModelWeightEditor from '../components/settings/TrendModelWeightEditor';
...
const SettingsPage: React.FC = () => {
  ...
  const [trendWeightsValid, setTrendWeightsValid] = useState(true);
  ...
  const canSaveCurrentConfig = activeCategory !== 'agent' || trendWeightsValid;
  ...
  const activeItems =
    activeCategory === 'ai_model'
      ...
      : activeCategory === 'agent'
        ? rawActiveItems.filter((item) => !AGENT_HIDDEN_KEYS.has(item.key) && !TREND_MODEL_WEIGHT_KEYS.includes(item.key as typeof TREND_MODEL_WEIGHT_KEYS[number]))
      : rawActiveItems;
  ...
  const activeConfigPanel = hasActiveConfigItems ? (
    ...
    >
      {activeCategory === 'agent' ? (
        <div className="mb-4">
          <TrendModelWeightEditor
            items={rawActiveItems}
            disabled={isSaving || isLoading}
            onChange={setDraftValue}
            onValidityChange={setTrendWeightsValid}
          />
        </div>
      ) : null}
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              disabled={!effectiveHasDirty || isSaving || isLoading || !canSaveCurrentConfig}
              isLoading={isSaving}
