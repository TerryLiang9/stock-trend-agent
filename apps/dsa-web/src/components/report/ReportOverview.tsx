...
import { trendForecastApi, type LatestTrendForecast } from '../../api/trendForecast';
import { TrendForecastBadge } from '../trend';
...
export const ReportOverview: React.FC<ReportOverviewProps> = ({
  ...
  // 获取趋势预测 Agent 的结构化裁决结果（补充 LLM 自由文本的方向信息）
  const [trendLatest, setTrendLatest] = useState<LatestTrendForecast | null>(null);
  useEffect(() => {
    let cancelled = false;
    trendForecastApi.getLatest(meta.stockCode).then((data) => {
      if (!cancelled) setTrendLatest(data);
    }).catch(() => {});
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  {trendLatest && !trendLatest.notFound && trendLatest.adjudication && (
                    <TrendForecastBadge adjudication={trendLatest.adjudication} />
                  )}
                </div>
              </div>
            </Card>
