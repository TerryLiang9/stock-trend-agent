...
import { Activity, BarChart3, Home, LayoutDashboard, LogOut, MessageSquareQuote, Search, Settings2 } from 'lucide-react';
...
const NAV_GROUPS: NavGroup[] = [
  ...
  {
    groupKey: 'layout.navGroup.analysis',
    items: [
      ...
      // trend-forecast 已从导航隐藏，路由和代码保留
      { key: 'trend-dashboard', labelKey: 'layout.nav.trendDashboard', to: '/trend-dashboard', icon: LayoutDashboard },
    ],
  },
  {
    groupKey: 'layout.navGroup.system',
    items: [
...
export const SidebarNav: React.FC<SidebarNavProps> = ({ collapsed = false, onNavigate, variant = 'default' }) => {
  ...
  const navGroups = NAV_GROUPS.map((group) => ({
    ...group,
    items: showAlphaSiftNav ? group.items : group.items.filter((item) => item.key !== 'screening'),
  })).filter((group) => group.items.length > 0);
  const isRail = variant === 'rail';
  const itemBaseClass = cn(
    'group relative flex h-[var(--nav-item-height)] w-full items-center rounded-2xl border border-transparent text-sm leading-[1.35] text-secondary-text transition-all',
    isRail
  ...
  const itemLabelClass = cn('min-w-0 truncate leading-[1.35]', isRail ? 'text-center' : '');
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      >
        ...
        {!collapsed ? (
          <p className={cn('min-w-0 truncate font-semibold leading-[1.35] text-foreground', isRail ? 'text-[0.95rem]' : 'text-sm')}>xuanling</p>
        ) : null}
      </div>

      <nav className={cn('flex flex-col gap-1.5', isRail ? '' : 'flex-1')} aria-label={t('layout.mainNav')}>
        {navGroups.map((group, gi) => {
          const groupLabel = t(group.groupKey as UiTextKey);
          return (
            <div key={group.groupKey} className={gi > 0 && !collapsed ? 'mt-2.5 pt-2 border-t border-border/40' : ''}>
              {!collapsed && groupLabel ? (
                <p className="mb-1 px-1 text-[10px] font-medium uppercase tracking-widest text-muted-text select-none">
                  {groupLabel}
                </p>
              ) : null}
              {group.items.map(({ key, labelKey, to, icon: Icon, exact, badge }) => {
