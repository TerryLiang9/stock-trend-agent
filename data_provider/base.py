...
def normalize_stock_code(stock_code: str) -> str:
    ...
    Non-A-share market symbols are no longer normalized here.
    """
...
def _market_tag(code: str) -> str:
    """返回市场标签。A 股-only 版本固定为 cn。"""
    return "cn"


def _is_hk_market(code: str) -> bool:
    """A 股-only 版本不再识别港股代码。保留函数名用于旧 fetcher 兼容。"""
    return False
...
class DataFetcherManager:
    ...
    def _init_default_fetchers(self) -> None:
        ...
        # 公司 L2 数据源（P0，仅当 collector 数据存在时启用）
        company_l2 = CompanyL2Fetcher()
        if company_l2.is_available():
            logger.info("[数据源初始化] 公司 L2 数据源可用，将作为 P0 优先使用")
        else:
            logger.debug("[数据源初始化] 公司 L2 数据源暂无数据，跳过（collector 运行后自动生效）")
    ...
    def get_daily_data(
        ...
        if total_fetchers == 0:
            error_summary = f"A股 {stock_code} 获取失败:\n暂无可用数据源"
