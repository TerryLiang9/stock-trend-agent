...
> 🤖 基于 AI 大模型的 A 股自选股智能分析系统，聚焦沪深北 A 股与 A 股 ETF，每日自动分析并推送「决策仪表盘」到企业微信/飞书/Telegram/Discord/Slack/邮箱。

[**产品预览**](#-产品预览) · [**功能特性**](#-功能特性) · [**快速开始**](#-快速开始) · [**推送效果**](#-推送效果) · [**A 股功能清单**](docs/a-share-features.md)
...
| A 股数据聚合 | 仅保留沪深北 A 股与 A 股 ETF，支持行情、K 线、技术指标、新闻、公告、基本面与报告辅助数据；保留能力见 [A 股功能清单](docs/a-share-features.md) |
...
| 智能导入与补全 | 图片、CSV/Excel、剪贴板导入；A 股代码/名称/拼音/别名补全 |
...
| 行情数据 | [TickFlow](https://tickflow.org/auth/register?ref=WDSGSPS5XC)、AkShare、Tushare、Pytdx、Baostock、Efinance |
...
> 当前版本仅保留 A 股数据源与运行入口。项目默认内置 AkShare、Baostock、Efinance 等免费 A 股行情源，可零配置运行；免费源受上游限流、接口变动和网络波动影响，稳定性不保证。长期定时、批量分析或更稳定行情建议配置 TickFlow、Tushare。

## 🗂️ 目录结构与作用

| 目录 / 文件 | 作用 |
|------|------|
| `main.py` | 本地分析、定时任务、Dry Run、市场复盘和服务模式的主入口 |
| `server.py` | FastAPI 服务启动入口 |
| `api/` | 后端 API 路由，包含 A 股分析、配置、历史记录和趋势 Agent 接口 |
| `src/core/` | 股票分析主流程编排，负责串联数据、分析、报告和通知 |
| `src/services/` | 业务服务层，封装分析任务、配置、报告、认证等能力 |
| `src/repositories/` | 数据访问层，负责历史记录、任务状态、配置等持久化读写 |
| `src/schemas/` | 后端数据结构和接口 Schema |
| `src/agent_system/` | A 股趋势 Agent 模块，负责行情/新闻快照、均线与研究模型、裁决、反馈和交易风控 |
| `researcher_strategy/` | 真实研究策略代码，包括小波盘前、历史相似日、六因子逻辑回归模型 |
| `data_provider/` | A 股行情、K 线、新闻、公告、基本面等数据源适配与降级 |
| `reports/` | 生成后的分析报告和运行产物目录 |
| `bot/` | 仅保留核心模块导入所需的 `BotMessage` 兼容类型，不包含 Bot 平台或推送功能 |
| `apps/dsa-web/` | Web 工作台前端 |
| `apps/dsa-desktop/` | Electron 桌面端 |
| `strategies/` | Agent 策略问股配置和策略说明 |
| `templates/` | 报告、页面或消息渲染模板 |
| `static/` | Web 静态资源目录 |
| `scripts/` | 本地运行、构建、验证、部署辅助脚本 |
| `docs/` | 配置指南、专题说明、变更记录和 A 股功能清单 |
| `tests/` | Pytest 自动化测试 |
| `requirements.txt` | A 股核心运行依赖 |
| `requirements-trend-forecast.txt` | A 股趋势 Agent 的 ClickHouse、研究模型和 LangGraph 可选依赖 |
| `requirements-alphasift.txt` | 可选 AlphaSift 研究依赖，网络不可用时不影响核心安装 |
...
| Secret 名称 | 说明 | 必填 |
|------------|------|:----:|
| `STOCK_LIST` | A 股自选股代码，如 `600519,300750,002594` | ✅ |
...
| Secret 名称 | 说明 | 必填 |
|------------|------|:----:|
| `ANSPIRE_API_KEYS` | [Anspire AI Search](https://open.anspire.cn/dsa?share_code=QFBC0FYC)：汇聚舆情信息，适配 A 股新闻和舆情检索；同一 Key 可复用大模型服务，本项目新用户提供免费 30 元等额的免费点数 | **推荐** |
...
| `BRAVE_API_KEYS` | [Brave Search](https://brave.com/search/api/)：隐私优先的通用搜索补强 | 可选 |
...
> 默认使用 AkShare、Baostock、Tushare、Pytdx 等 A 股数据源，日志中"未配置"的提示不影响运行。
> 如需更稳定的 A 股行情，可配置以下 Secret：

| Secret 名称 | 说明 |
|------------|------|
| `TUSHARE_TOKEN` | 提升 A 股历史行情稳定性 |
...
默认每个 **A 股交易日 18:00（北京时间）** 自动执行，也可手动触发；强制运行、交易日检查、断点续传等规则见 [完整指南](docs/full-guide.md#定时任务配置)。
