...
# 模糊匹配：LLM 自由文本中如果包含这些子串，映射到五档
_TREND_FUZZY_KEYWORDS: list[tuple[str, str]] = [
    ("强烈看多", "strong_bullish"),
    ("强势多头", "strong_bullish"),
    ("强势看多", "strong_bullish"),
    ("强烈看空", "strong_bearish"),
...
def localize_trend_prediction(value: Any, language: Optional[str]) -> str:
    """\u5c06\u8d8b\u52bf\u9884\u6d4b\u6587\u672c\u5f52\u4e00\u5316\u5230\u4e94\u6863\uff1a\u5f3a\u70c8\u770b\u591a/\u770b\u591a/\u9707\u8361/\u770b\u7a7a/\u5f3a\u70c8\u770b\u7a7a\u3002

    \u5373\u4f7f LLM \u8f93\u51fa\u4e86\u81ea\u7531\u6587\u672c\uff08\u5982"\u5f31\u52bf\u7a7a\u5934\uff0c\u8d85\u8dcc\u4fee\u590d\u975e\u53cd\u8f6c"\uff09\uff0c\u4e5f\u4f1a\u901a\u8fc7
    \u6a21\u7cca\u5173\u952e\u8bcd\u5339\u914d\u56de\u843d\u5230\u4e94\u6863\u4e4b\u4e00\uff0c\u786e\u4fdd\u9996\u9875\u5c55\u793a\u7684\u7ed3\u8bba\u6c38\u8fdc\u6e05\u6670\u53ef\u8fa8\u3002
    """
    normalized_language = normalize_report_language(language)
    raw_text = str(value or "").strip()
    if not raw_text:
    ...
    # 1. \u5148\u5c1d\u8bd5\u7cbe\u786e\u5339\u914d canonical map
    canonical = _canonicalize_lookup_value(raw_text, _TREND_PREDICTION_CANONICAL_MAP)
    if canonical:
        return _TREND_PREDICTION_TRANSLATIONS[canonical][normalized_language]

    # 2. \u4e2d\u6587\u6a21\u7cca\u5339\u914d\uff1a\u5728 LLM \u81ea\u7531\u6587\u672c\u4e2d\u627e\u4e94\u6863\u5173\u952e\u8bcd
    if re.search(r"[\u4e00-\u9fff]", raw_text):
        for keyword, canon_key in _TREND_FUZZY_KEYWORDS:
            if keyword in raw_text:
                return _TREND_PREDICTION_TRANSLATIONS[canon_key][normalized_language]
        # 3. \u5b8c\u5168\u65e0\u6cd5\u8bc6\u522b\u65f6\uff0c\u56de\u843d\u5230\u82f1\u6587\u7ffb\u8bd1\u8def\u5f84\uff08keyword matching inside _translate_from_map\uff09
        #    \u6700\u7ec8\u4ecd\u7136\u515c\u5e95\u4e3a\u539f\u59cb\u6587\u672c
