...
class SystemConfigService:
            ...
            try:
                ...
                with open_hermes_no_proxy_client(
                    ...
                    call_kwargs = cls._build_llm_capability_completion_kwargs(
                        ...
                        base_url=base_url,
                        timeout_seconds=timeout_seconds,
                        messages=[{"role": "user", "content": '请只返回这个 JSON 对象：{"status":"ok"}'}],
                        max_tokens=64,
    ...
    def _validate_cross_field(effective_map: Dict[str, str], updated_keys: Set[str]) -> List[Dict[str, Any]]:
        ...
        trend_weight_keys = {
            "MA_AGENT_WEIGHT_MOVING_AVERAGE",
            "MA_AGENT_WEIGHT_WAVELET",
            "MA_AGENT_WEIGHT_ANALOG",
            "MA_AGENT_WEIGHT_LOGISTIC_6F",
        }
        if trend_weight_keys & updated_keys:
            parsed_weights: Dict[str, float] = {}
            key_to_name = {
                "MA_AGENT_WEIGHT_MOVING_AVERAGE": "moving_average",
                "MA_AGENT_WEIGHT_WAVELET": "wavelet",
                "MA_AGENT_WEIGHT_ANALOG": "analog",
                "MA_AGENT_WEIGHT_LOGISTIC_6F": "logistic_6f",
            }
            defaults = {
            ...
            for key, name in key_to_name.items():
                raw = (effective_map.get(key) or defaults[key]).strip()
                try:
                    parsed_weights[name] = float(raw)
                except (TypeError, ValueError):
                    continue
            if len(parsed_weights) == len(key_to_name):
                total = sum(parsed_weights.values())
                if abs(total - 1.0) > 1e-6:
                    issues.append({
                        "key": "MA_AGENT_WEIGHT_MOVING_AVERAGE",
                        "code": "trend_weight_total_invalid",
                        "message": "四个 A 股趋势模型权重之和必须为 1.0。",
                        "severity": "error",
                        "expected": "sum=1.0",
                        "actual": total,
                    })
                research_max = max(parsed_weights[name] for name in ("wavelet", "analog", "logistic_6f"))
                if parsed_weights["moving_average"] <= research_max:
                    issues.append({
                        "key": "MA_AGENT_WEIGHT_MOVING_AVERAGE",
                        "code": "trend_weight_ma_not_dominant",
                        "message": "均线基线权重必须大于任一研究模型权重。",
                        "severity": "error",
                        "expected": "moving_average > max(research_models)",
                        "actual": parsed_weights["moving_average"],
                    })
