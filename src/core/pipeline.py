...
class StockAnalysisPipeline:
    ...
    def fetch_and_save_stock_data(
        ...
        try:
            ...
            df, source_name = self.fetcher_manager.get_daily_data(code, days=self.config.analysis_lookback_days)
    ...
    def analyze_stock(
        ...
        try:
            ...
            try:
                ...
                # 趋势分析回溯天数：至少 89 天（~60 交易日），或按配置的 2 倍（覆盖更长周期）
                _trend_lookback = max(89, self.config.analysis_lookback_days * 2)
                start_date = end_date - timedelta(days=_trend_lookback)
    ...
    def run(
        ...
        if len(stock_codes) >= 5:
            daily_prefetch_count = self.fetcher_manager.prefetch_daily_klines(stock_codes, days=self.config.analysis_lookback_days)
