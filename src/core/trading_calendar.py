...
def get_market_for_stock(code: str) -> Optional[str]:
    ...
    Returns:
        'cn' | None (None = unrecognized/non-A-share)
    """
...
def get_next_trading_date(
    market: Optional[str], current_date: Optional[date] = None
) -> date:
    """返回指定日期之后的下一个交易日；日历不可用时只跳过周末。"""
    candidate = current_date or get_market_now(market).date()
    if _XCALS_AVAILABLE and MARKET_EXCHANGE.get(market or ""):
        try:
            cal = xcals.get_calendar(MARKET_EXCHANGE[market or ""])
            return cal.next_session(candidate).date()
        except Exception as exc:
            logger.warning("trading_calendar.get_next_trading_date fallback: %s", exc)
...
def get_open_markets_today() -> Set[str]:
    ...
    Returns:
        Set containing ``cn`` when A-shares are trading today.
    """
    if not _XCALS_AVAILABLE:
...
def compute_effective_region(
    ...
    Args:
        config_region: From MARKET_REVIEW_REGION. Only ``cn`` is supported.
    ...
    Returns:
        ...
        'cn': A-share market review region
    """
    normalized = (config_region or "cn").strip().lower()
    if normalized and normalized != "cn":
        logger.warning("MARKET_REVIEW_REGION=%s is ignored; only cn is supported", config_region)
