...
class Config:
    ...
    analysis_lookback_days: int = 30  # 日线数据回溯天数（ANALYSIS_LOOKBACK_DAYS）
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            analysis_lookback_days=parse_env_int(
                os.getenv('ANALYSIS_LOOKBACK_DAYS'),
                30,
                field_name='ANALYSIS_LOOKBACK_DAYS',
                minimum=1,
            ),
    ...
    def _parse_market_review_region(cls, value: str) -> str:
        """解析大盘复盘市场区域；A 股-only 版本固定为 cn。"""
        import logging
        v = (value or 'cn').strip().lower()
        if v and v != 'cn':
            logging.getLogger(__name__).warning(
                "MARKET_REVIEW_REGION 配置值 '%s' 已忽略；当前版本仅保留 A 股，固定使用 cn。",
                value,
            )
        return 'cn'
    ...
    def validate_structured(self) -> List[ConfigIssue]:
        ...
        if not self.stock_list:
            issues.append(ConfigIssue(
                severity="error",
                message="未配置 STOCK_LIST。请设置至少一个 A 股 6 位股票代码，例如：600519,300750,002594。",
                field="STOCK_LIST",
            ))
