...
def _build_stock_code_lookup(raw_items: list) -> Dict[str, str]:
    ...
    for item in raw_items:
        ...
        if not (canonical_upper.isdigit() and len(canonical_upper) == 6):
            continue
    ...
    result: Dict[str, str] = {}
    for key, codes in exact_lookup.items():
        if len(codes) == 1:
...
def resolve_index_stock_code(query: str) -> str | None:
    ...
    A-share-only build resolves six-digit A-share canonical/display-code
    matches and ignores offshore index records.
    """
