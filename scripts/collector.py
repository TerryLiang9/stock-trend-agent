...
# ---------------------------------------------------------------------------
def _ob(obj: dict) -> dict:
    """安全取 orderbook：实盘偶尔推成 list/None，统一成 dict。"""
    ob = obj.get("orderbook")
...
def parse_snapshot(sym: str, obj: dict, recv_ts: int) -> dict:
    ...
    bid_p, ask_p = _lst(ob.get("bidPrice")), _lst(ob.get("askPrice"))
    bid_v, ask_v = _lst(ob.get("bidVol")), _lst(ob.get("askVol"))
    ...
    def handle_message(self, data: dict, recv_ts: int):
        ...
        for sym in self.symbols:
            ...
            try:
                ...
                for t in _lst(obj.get("l2_trade")):
                ...
                for o in _lst(obj.get("l2_order")):
...
def main():
    ...
    ap.add_argument("--out", default="./data", help="落地根目录")
